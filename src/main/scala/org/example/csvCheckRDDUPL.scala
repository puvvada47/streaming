...
object csvCheckRDDUPL {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val lst1 = List("Bala","Krishna","Sandeep","Kumar")
    //val lst2 = List([5, 3, [9,0,4], 7], 'Naresh', [], [8, 6]])
    val spark = SparkSession.builder
      .config(sparkConf).getOrCreate()
    import spark.implicits._
    val rdd: RDD[String] =spark.sparkContext.parallelize(lst1)
    val d1=rdd.toDF()
    d1.show()
